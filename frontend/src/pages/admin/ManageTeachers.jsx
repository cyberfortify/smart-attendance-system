...
export default function ManageTeachers() {
  ...
  async function load() {
    setIsLoading(true);
    try {
      const [tRes, cRes, mRes, sRes] = await Promise.all([
  ...
  async function removeMapping(id) {
    try {
      ...
      setToast({ message: "Assignment removed successfully", variant: "success" });
      await load();
    } catch (err) {
  ...
  async function handleAssignClass() {
    ...
    setIsLoading(true);
    try {
      await api.post("/admin/teacher-subjects", assignForm);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="h-[calc(100vh-200px)] flex flex-col">
            ...
            {activeTab === "teachers" && (
              <div className={`${glassCard} flex-1 overflow-y-auto`}>
                ...
                <div className="p-4 border-b border-slate-200/60 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-slate-600 mt-1">
                      {mappings.length} active assignments
                    </p>
                  </div>
                ...
                {mappings.length > 0 ? (
                  <div className="divide-y divide-slate-100/50 overflow-y-auto">
                    {mappings.map((m) => (
                      <div
                        key={m.id}
                        className="p-4 hover:bg-slate-50/80 transition-all border-l-4 border-transparent hover:border-purple-400/50"
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-start gap-4 flex-1">

                            <div className="w-10 h-10 rounded-lg bg-gradient-to-r from-purple-100 to-pink-100 flex items-center justify-center shadow-sm">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="flex flex-wrap gap-2 mt-2">
                                <div className="flex items-center gap-1 bg-indigo-50 text-indigo-700 px-3 py-1 rounded-full text-xs font-medium border border-indigo-100">
                                  <BookOpen className="w-3 h-3" />
                                  {m.subject_name}
                                </div>

                                {m.section && (
                                  <div className="flex items-center gap-1 bg-slate-100 px-3 py-1 rounded-full text-xs text-slate-600 border border-slate-200">
                                    Section: {m.section}
                                  </div>
                                )}

                                <div className="flex items-center gap-1 bg-slate-100 px-3 py-1 rounded-full text-xs text-slate-600 border border-slate-200">
                                  Teacher ID: {m.teacher_id}
                                </div>
                              </div>
                            </div>
                          </div>
                          ...
                          <button
                            ...
                            className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all shadow-sm ml-2"
                  ...
                  <div className="p-12 text-center">
                    ...
                    <p className="text-sm text-slate-600 mb-4 max-w-lg mx-auto">
                      Assign subjects to teachers to enable subject-wise attendance tracking.
                    </p>

                    <button
                      onClick={() => setShowAssignModal(true)}
                      className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-emerald-500 to-emerald-400 text-white rounded-lg font-medium"
                    >
                      <Link className="w-4 h-4" />
                      Create Assignment
                    </button>
            ...
            {activeTab === "create" && (
              <div className={`${glassCard} flex-1 overflow-y-auto`}>
                <div className="flex items-center gap-3 mb-4 p-4 ">
                  ...
                  <div>
                    <h2 className="text-xl font-semibold text-slate-900">Create New Teacher</h2>
                ...
                <div className="flex items-center justify-between mt-4 p-4 border-t border-slate-200/60">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className={`${glassCard2} p-4 sm:p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto`}>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <select
                  ...
                  className="w-full pl-10 pr-10 py-2 rounded-xl border border-slate-200 bg-white/80 outline-none text-sm"
                >
                  <option value="">Choose a subject</option>
                  {subjects
                    .filter(s => s.class_id == assignForm.class_id)
                    .map(s => (
            ...
            <div className="flex gap-3 pt-3 border-t border-slate-200/60">
              ...
              <button onClick={handleAssignClass} disabled={!assignForm.teacher_id || !assignForm.class_id || !assignForm.subject_id} className={`flex-1 py-2 rounded-xl text-white ${!assignForm.teacher_id || !assignForm.class_id ? "bg-slate-100 text-slate-400" : "bg-gradient-to-r from-purple-500 to-indigo-500"}`}>Assign Class</button>
            </div>
          </div>
        </div>
      )}
